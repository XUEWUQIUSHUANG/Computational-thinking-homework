...
void information(Person *person1)   
{
	...
	FILE *FBI;
	FBI=fopen("C:\\远程交互系统\\服务器register.txt","w"); 
...
void coutinformation(Person *person1)  
{
	...
	{
		FILE *FBI;
		FBI=fopen("C:\\远程交互系统\\服务器register.txt","r"); 
		char ch[6][20]={0};
		for(int i=0;i<6;i++)
		{
			fscanf(FBI,"%s",ch[i]);
...
here1:	
	...
	if(judge(tail,(char*)".jpg")||judge(tail,(char*)".exe")||judge(tail,(char*)".zip")||judge(tail,(char*)".rar")||judge(tail,(char*)".png")||judge(tail,(char*)".txt")||judge(tail,(char*)".doc")||judge(tail,(char*)".mp3")||judge(tail,(char*)".mp4")||judge(tail,(char*)".abr")||judge(tail,(char*)".pdf")) 
	{
		...
		int fsize;
		int size=0;
		char str1[99]="C:\\远程交互系统\\服务器接收文件夹\\" ;
		char str2[5];
		itoa(P_name,str2,10);
		...
		char mem[5];
		int c;
		fp1=fopen("C:\\远程交互系统\\服务器接收文件夹\\计数器.txt","r");
		...
		if(P_name>c)
		{
			fp2=fopen("C:\\远程交互系统\\服务器接收文件夹\\计数器.txt","w");
			fputs(str2,fp2);
			fclose(fp2);
		}
		else
	...
	else
	{
		...
		//打开并存入聊天信息 
		Dialog=fopen("C:\\远程交互系统\\服务器dialog.txt","a+");
		fprintf(Dialog,"对方：");
...
here2:	
	...
	{
		Dialog=fopen("C:\\远程交互系统\\服务器dialog.txt","w+");
		fclose(Dialog);
		for(int i=5;i<21;i++)
		{
			SetXY(102,i);
...
{ 
	...
	bind(seversocket, (SOCKADDR*)&socketaddr, sizeof(socketaddr));
	
	//  进入监听设备
	listen(seversocket, 20);
